...
<title>Search Result | Jibika - Free classifieds in India, Buy and Sell for free anywhere in India with Jibika Online Classified Advertising</title>
...
$search_result=app('request')->input('search_result');


$city_value=DB::table('cities')->where('keyword',$city)->first();


$subcategory=DB::table('subcategories')->where('subcategory','LIKE',"%{$search_result}%")->first();
$subcategory_count=DB::table('subcategories')->where('subcategory','LIKE',"%{$search_result}%")->count();

if($subcategory_count>0)
{
$ad_count=DB::table('ads')
                    ->where('city',$city_value->id)
                    ->where('subcat_id',$subcategory->id)
...
<section class="top_listings pb-4">
  ...
  <div class="row justify-content-center">
    <div class="col-md-7 text-center">
      <h4 class="title">Searching Result : {{ $search_result }} In {{$city_value->city}}</h4>
    </div>
  </div>
  <!-- Row  -->
  <div class="row">
                    ...
                    <select class="form-control text-truncate" name="city" required>
                      ...
                      @php
                       ...
                       <div class="form-group">
                        <input type="text" class="form-control text-truncate" placeholder="Find Cars, Mobile Phones and more..." value="{{$search_result}}" name="search_result" required>
                      </div>
                  <div class="form-group">
 ...
 @if ($subcategory_count>0 && $ad_count>0)
