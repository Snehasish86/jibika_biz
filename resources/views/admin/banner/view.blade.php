                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header">
                            <h5 class="card-title mb-0">Banner List <a href="{{ url('/admin/banner/create') }}"><button class="btn btn-primary">Add+</button></a></h5>
                                ...
                                <tbody>
                                    ...
                                    @foreach ($row as $details)
                                        ...
                                        <tr>
                                            ...
                                            <td>{{$details->url_link}}</td>
                                            <td>{{$details->start_date}}</td>
                                            <td>{{$details->end_date}}</td>
                                            <td>{{$details->active_status}}</td>
                                            <td><a href="{{url('/admin/banner/'.$details->id.'/edit')}}"><i data-feather="edit"></i></a></td>
                                            <td><i data-feather="archive" onclick="delete_row({{$details->id}})"></i></td>
                                            <form action="{{url('/admin/banner/'.$details->id)}}" method="post" id="delete_submit{{$details->id}}" style="display:none;">
                                                    {{ method_field('DELETE') }}
                                                        {{csrf_field()}}
